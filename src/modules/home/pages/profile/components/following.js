...
export default function Following() {
    ...
    const removeFollowing = catchAsync(async (id) => {
    ...
    return(
        <Box sx={{border: isBorder ? '1px solid red' : 'none', width: '100%'}}>
            <Stack spacing={2}>
                {
                    user.following.map((node, index) => (
                        <Stack key={index} direction='row' justifyContent='space-between'>
                            <Stack direction='row' spacing={2} alignItems='center'>
                            {getUserAvatar(node, purple, 38, 38, 20)}
                                <Stack>
                            ...
                            {
                                removeBool &&
                                <IconButton fontSize='small' onClick={() => removeFollowing(node._id)}>
